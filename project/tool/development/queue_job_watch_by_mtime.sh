...
find $ROOT_DIR/command/queue_job/ -type f -print0 | xargs -0 stat -c '%y %n' > $OLD_MD5_FILE

while true
do
    sleep 1

    find $ROOT_DIR/command/queue_job/ -type f -print0 | xargs -0 stat -c '%y %n' > $NEW_MD5_FILE
